...
(deftest test-output-interception
  (testing "callbacks are invoked when output is written"
    ...
    (let [out-calls (atom [])
          err-calls (atom [])]
      (out/add-callback :out :test1 #(swap! out-calls conj [:test1 %]))
      (out/add-callback :out :test2 #(swap! out-calls conj [:test2 %]))
      (out/add-callback :err :test3 #(swap! err-calls conj [:test3 %]))
...
(deftest test-error-handling-in-callbacks
  (testing "exceptions in one callback shouldn't break others"
    (out/wrap-standard-streams)
    (let [out-calls (atom [])]
      (out/add-callback :out ::bad-callback #(/ 1 %))
      (out/add-callback :out ::good-callback #(swap! out-calls conj %))
