...
(defn tty-read-msg
  "Invokes read on r, the provided java.io.PushbackReader.  The current namespace cns
  and session id (provided as atoms) are used with the read result to return an eval op."
  [r cns session-id]
  ...
  ([in out & [^Closeable s]]
   (let [r (PushbackReader. (io/reader in))
         ...
         prompt (fn [newline?]
                  ...
                  (.write w (str @cns "=> ")))
         session-id (atom nil)
         read-seq (atom (cons {:op "clone"} (repeatedly tty-read-msg)))
