...
(defn- gather-initial-bindings
  ...
  (as-> (transient
    ...
    ;; Bindings required for other nRPEL middleware to work.
...
(defn make-ephemeral-exec-fn
  "Return an exec function that should attached to ephemeral sessions. Ephemeral
  exec fn does not persist dynamic bindings and uses `default-executor`. The
  submitted task is made of:
...
(defn session-exec
  ...
  - `:exec` - takes an id (typically a msg-id), a thunk and an ack runnables,
    and an optional messsage (see `make-ephemeral-exec-fn` for ampler context).
  ...
  (let [id (:id (meta session))
    ...
    {:interrupt (fn [exec-id]
     ...
     :exec (fn [exec-id r ack & [msg]]
             (.put (:queue @state) [exec-id r ack msg]))}))
