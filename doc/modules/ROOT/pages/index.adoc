= nREPL
:notitle:
:description: nREPL is a network REPL for Clojure: a server, a client and a simple protocol that editors and tools build on.

image::logo-w1280.png[nREPL's logo,480]
...
* xref:installation.adoc[Installation] to add nREPL to a project.
* xref:usage/server.adoc[Server] to start and configure an nREPL server.
* xref:usage/clients.adoc[Clients] to connect an editor, a command-line client or your own code.
* xref:design/overview.adoc[Design] to understand the protocol before building on it.
