= Building Middleware
:description: How to write nREPL middleware that adds operations to a server, with testing and best practices.
...
This separation makes it easier to test the business logic in isolation and to reuse it outside
...
The example above uses `response-for` to build a response map and `t/send` to
send it manually.  For most cases, `nrepl.transport/respond-to` is a more
convenient alternative -- it extracts the transport from the message, builds the
response with `response-for`, and sends it in one step:
...
;; Instead of:
(t/send (:transport msg) (response-for msg {:status :done :value result}))

;; You can write:
(t/respond-to msg :status :done :value result)

;; Or pass a map:
(t/respond-to msg {:status :done :value result})
----

`respond-to` accepts the same arguments as `response-for` (key-value pairs
or maps) and is used extensively throughout nREPL's built-in middleware.

=== Robustness and error handling

Given how misbehavior of a foundational tool like nREPL can greatly diminish the
user experience, it is doubly important to prevent exceptions or errors from
happening during middleware processing, but also vital to signalize that an error
...
to the client via `respond-to`. If `(foo-reply msg)` throws an exception, a
message with `:status #{:foo-error :error :done}` will be sent to the client.

When using `safe-handle`, the handler function (e.g. `foo-reply`) should return
