= Alternatives
:description: How nREPL compares to the socket REPL, prepl and the Language Server Protocol.
...
built into Clojure since version 1.10. Clients feed `prepl` text to be evaluated
...
references and the like into the tool.
...
The most popular LSP server for Clojure is https://github.com/clojure-lsp/clojure-lsp[clojure-lsp].
Its functionality is powered internally by static code analysis, but one can easily imagine
