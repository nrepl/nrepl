= Installation
:description: Adding nREPL to a Leiningen, tools.deps, Gradle or Maven project, and enabling the JVMTI agent on Java 21+.
...
nREPL is available in https://clojars.org/nrepl[Clojars]. Add it as a
dependency with your build tool of choice:
...
TIP: Adding nREPL as a dependency also overrides the version that Leiningen
itself ships.
--
...
dependencies {
  ...
  implementation 'nrepl:nrepl:{nrepl-version}'

  // or to override the version of nREPL used by the clojureRepl task
  nrepl 'nrepl:nrepl:{nrepl-version}'
}
----
--
