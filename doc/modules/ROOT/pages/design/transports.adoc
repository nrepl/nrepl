...
comprising only two scalar types (byte strings and integer) and two
...
You'll need a bencode-capable client to connect to an nREPL server using that transport.
...
The main difference between the bencode transport and the EDN one is that instead of bencode dictionaries and lists, and being limited to integer and byte string types, you'd be sending and receiving full EDN structures and types, including maps, vectors, lists, sets, strings, keywords and symbols. The structure of the messages is similar, only the data format changes.

This may be useful in a couple of use cases: In some clients, including ClojureScript ones, where EDN is easier to handle than bencode. Furthermore, because this transport exposes more of the richer data types in the internals of nREPL and its middleware, it may support more complex use cases.

The EDN transport exposes more of the nREPL message format than bencode does.
Notably, `:op` values are strings (e.g., `"eval"`, not `:eval`), consistent with
how ops are handled internally and by third-party middleware. `:status` is either
a keyword or a set of keywords. These details are reflected in the `nrepl.spec`
ns, against which responses are verified during testing.
