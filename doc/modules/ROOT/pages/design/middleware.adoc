...
* `:requires`, a set containing strings or vars identifying other middleware
  that must be applied at a higher level ("before") than the middleware being
  described. Var references indicate a dependency on exact middleware; string
  values indicate a dependency on _any_ middleware that handles the specified
  `:op`.
* `:expects`, the same as `:requires`, except the referenced middleware must
  exist in the final stack at a lower level ("after") than the middleware being
  described.
...
`:expects` but this will not automatically pull them into middleware stack. All
desired middleware must be listed explicitly during handler construction,
otherwise an exception will be thrown by
...
that middleware will be executed before any middleware that handles the `"eval"`
operation, but after the `nrepl.middleware.session/session`
...
sequence of middleware vars) should make sure to add `wrap-desribe`, and also
