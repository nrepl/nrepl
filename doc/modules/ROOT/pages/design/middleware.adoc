...
* `:requires`, a set containing strings or vars identifying other middleware
  that must process a request _before_ the middleware being described sees it.
* `:expects`, a set containing strings or vars identifying other middleware
  that must process a request _after_ the middleware being described.
+
...
** *String references* (e.g., `"eval"`) create a dependency on _any_ middleware
   ...
   when you depend on an _operation_ being available rather than a specific
...
relative to it _when both are present_, use a *string reference* to an op that
