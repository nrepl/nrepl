= nREPL Clients
:description: Connecting to an nREPL server from editors, command-line clients, libraries, the built-in client or your own code.
...
----
$ clj -Sdeps '{:deps {nrepl/nrepl {:mvn/version "{nrepl-version}"}}}' -M -m nrepl.cmdline --interactive
...
[source,shell,subs="attributes+"]
----
$ clj -Sdeps '{:deps {nrepl/nrepl {:mvn/version "{nrepl-version}"}}}' -M -m nrepl.cmdline --connect --host host --port port
nREPL {nrepl-version}
Clojure 1.9.0
Java HotSpot(TM) 64-Bit Server VM 10.0.1+10
user=> (+ 1 2)
...
[source,shell,subs="attributes+"]
----
$ clj -Sdeps '{:deps {nrepl/nrepl {:mvn/version "{nrepl-version}"}}}' -M -m nrepl.cmdline --connect --socket /some/where/safe/nrepl
nREPL {nrepl-version}
Clojure 1.9.0
Java HotSpot(TM) 64-Bit Server VM 10.0.1+10
user=> (+ 1 2)
...
transport and the address both come from the URL, so `--transport`, `--port`
and `--socket` are refused when combined with one:

[source,shell,subs="attributes+"]
----
$ clj -Sdeps '{:deps {nrepl/nrepl {:mvn/version "{nrepl-version}"}}}' -M -m nrepl.cmdline --connect --host nrepl://127.0.0.1:59403
nREPL {nrepl-version}
Clojure 1.9.0
Java HotSpot(TM) 64-Bit Server VM 10.0.1+10
user=> (+ 1 2)
...
`nrepl+edn` (the EDN transport). A server listening on a filesystem socket
advertises a `nrepl+unix:` (or `nrepl+edn+unix:`) URL, which works the same
way.
...
[source,shell,subs="attributes+"]
----
$ clj -Sdeps '{:deps {nrepl/nrepl {:mvn/version "{nrepl-version}"}}}' -M -m nrepl.cmdline --connect --host nrepls://127.0.0.1:4001 --tls-keys-file client.keys
----
...
[source,shell,subs="attributes+"]
----
$ clj -Sdeps '{:deps {nrepl/nrepl {:mvn/version "{nrepl-version}"} nrepl/drawbridge {:mvn/version "{drawbridge-version}"}}}' -M -m nrepl.cmdline --connect --host https://your-app.example.com/repl
----
