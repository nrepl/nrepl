...
* https://github.com/mauricioszabo/atom-chlorine[Chlorine] (Atom plugin for Clojure Development)
...
[source,shell]
----
$ clj -Sdeps '{:deps {nrepl/nrepl {:mvn/version "1.5.2"}}}' -M -m nrepl.cmdline --connect --host host --port port
nREPL 1.5.2
Clojure 1.9.0
Java HotSpot(TM) 64-Bit Server VM 10.0.1+10
user=> (+ 1 2)
...
[source,shell]
----
$ clj -Sdeps '{:deps {nrepl/nrepl {:mvn/version "1.5.2"}}}' -M -m nrepl.cmdline --connect --socket /some/where/safe/nrepl
nREPL 1.5.2
Clojure 1.9.0
Java HotSpot(TM) 64-Bit Server VM 10.0.1+10
user=> (+ 1 2)
