...
Settings passed via the command-line interface will replace configuration
file settings by default. Since version 1.6, you can use the `^:concat` metadata
to concatenate collection values instead of replacing them.
...
The `^:concat` metadata can be used on either config values or CLI arguments,
and is useful for ensuring certain middleware (e.g., company-wide monitoring/metrics
middleware) is always loaded, even when development tools pass their own middleware
via CLI flags.

NOTE: The `^:concat` metadata only applies to top-level configuration.
...
Starting with version 1.6, middleware symbols can be marked with `^:optional` metadata to enable graceful
degradation. By default, the server will exit if middleware cannot be loaded.
With `^:optional`, unavailable middleware is skipped.
...
This is useful for configuring nREPL middleware references once, and not have to
change the configuration depending on the availability of the referenced
middleware on the classpath.
